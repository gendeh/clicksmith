...
def test_learned_hint_still_clicks_the_rerendered_120_percent_button():
    from app.main import build_scale_candidates

    near_recorded = build_scale_candidates(0.7, 1.4, 1.0)
    assert near_recorded[:7] == [1.0, 0.7, 1.4, 1.1, 0.9, 1.2, 0.67]
    learned = build_scale_candidates(0.7, 1.4, 1.12)
    assert learned[:4] == [1.12, 0.7, 1.4, 1.2]
    ...
    center_y = origin[1] + height / 2.0
    assert float(match["confidence"]) > 0.6
    assert abs(float(match["scale"]) - 1.2) <= 0.02
    assert abs(float(match["x"]) - center_x) <= 8
    assert abs(float(match["y"]) - center_y) <= 8
    assert int(data["processingTimeMs"]) <= 260
...
def test_region_ocr_reads_a_plain_line_outside_the_buttons():
    app = create_app()
    client = app.test_client()
    page = np.full((800, 1100, 3), (251, 247, 245), dtype=np.uint8)
    cv2.putText(page, "Filler 01", (210, 670), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (20, 20, 20), 1, cv2.LINE_AA)
    res = client.post(
        "/ocr",
        json={
    ...
    assert data["success"] is True
    words = {
        item["text"].strip(): item["bounds"]
        for item in data["items"]
        if " " not in item["text"].strip()
    }
    assert "Filler" in words
    assert "01" in words
...
def test_region_ocr_reads_a_line_that_crosses_the_neighborhood_edge():
    app = create_app()
    client = app.test_client()
    page = np.full((800, 1100, 3), (251, 247, 245), dtype=np.uint8)
    cursor = 40
    for word in "Use this page for deterministic testing. Keep the browser UI stable.".split(" "):
        cv2.putText(page, word, (cursor, 459), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (20, 20, 20), 1, cv2.LINE_AA)
        (width, _height), _baseline = cv2.getTextSize(word, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 1)
        cursor += width + 8
    res = client.post(
        "/ocr",
