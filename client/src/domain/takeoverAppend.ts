...
export interface TakeoverAppendReport {
  kept: number;
  replaced: number;
  appended: number;
  takeoverStartMs: number;
...
export function reportTakeoverAppend(
  baseEventCount: number,
  merged: RecordedEvent[],
  takeoverStartMs: number
): TakeoverAppendReport {
  const appendedEvents = merged.filter(event => event.human_override);
  const appended = appendedEvents.length;
  const kept = merged.length - appended;
  const firstHuman = appendedEvents[0];
  return {
    kept,
    replaced: Math.max(0, baseEventCount - kept),
    appended,
    takeoverStartMs,
    gapMs: firstHuman ? firstHuman.t_ms - takeoverStartMs : null,
  };
}
