...
liveMatch('a scrolled Coral word stays on the word when the window is not at the origin', async () => {
  const fs = require('fs') as typeof import('fs');
  const sharp = require('sharp');
  const imageService = new ImageService('http://127.0.0.1:5001');
  expect(await imageService.healthCheck(800)).toBe(true);
  ...
  const point = center(recordedBox);
  const captureSpy = jest.spyOn(screenCapture, 'captureRegion').mockImplementation(async (region) => {
    const localX = region.x - origin.x;
    const localY = region.y - origin.y;
    const left = Math.max(0, Math.min(frameW - 1, Math.round(localX)));
    const top = Math.max(0, Math.min(frameH - 1, Math.round(localY)));
    const width = Math.max(1, Math.min(frameW - left, Math.round(region.width)));
    const height = Math.max(1, Math.min(frameH - top, Math.round(region.height)));
    return sharp(shifted).extract({ left, top, width, height }).png().toBuffer();
  });
  ...
  const engine = new PlaybackEngine({
    ...
    imageService,
    windowManager: {
      getTargetBounds: () => ({ x: origin.x, y: origin.y, width: frameW, height: frameH }),
      getTargetBoundsAsync: async () => ({ x: origin.x, y: origin.y, width: frameW, height: frameH }),
    } as any,
  }) as any;
  engine.config = {
    profileId: 'live-scrolled-coral-origin',
    target: 'Terminal',
    useImageMatching: true,
    imageMatchThreshold: 0.6,
    timingTolerance: 20,
  ...
  captureSpy.mockRestore();
  screenSpy.mockRestore();
  const word = { x: 635, y: 521, w: 29, h: 9 };
  const anchored = {
    x: origin.x + word.x + word.w / 2 + 0.42 * word.w,
    y: origin.y + word.y + word.h / 2,
  };
  const nearWord =
    Math.abs(result.x - anchored.x) <= 12 && Math.abs(result.y - anchored.y) <= 12;
  if (!nearWord || elapsed >= 460 || status.smartClickLastMethod !== 'ocr' || !(status.smartClickLastConfidence > 0.6) || status.smartClickLastSource !== 'window') {
    throw new Error(
      `click (${result.x}, ${result.y}) anchored (${anchored.x.toFixed(1)}, ${anchored.y.toFixed(1)}) ${status.smartClickLastMethod} ${status.smartClickLastConfidence} source ${status.smartClickLastSource} elapsed ${elapsed}`
    );
  }
}, 30000);

liveMatch('a scrolled line still clicks the rare word inside the click wait', async () => {
  const fs = require('fs') as typeof import('fs');
  const sharp = require('sharp');
  const imageService = new ImageService('http://127.0.0.1:5001');
  expect(await imageService.healthCheck(800)).toBe(true);
  ...
  const started = Date.now();
  const result = await engine.getSmartClickCoords(0, event, point);
  const elapsed = Date.now() - started;
  const status = engine.getStatus();
  captureSpy.mockRestore();
  screenSpy.mockRestore();
  const word = { x: 649.5, y: 525.5 };
  const nearWord = Math.abs(result.x - word.x) <= 14 && Math.abs(result.y - word.y) <= 14;
  if (!nearWord || elapsed >= 460 || status.smartClickLastMethod !== 'ocr' || !(status.smartClickLastConfidence > 0.6) || status.smartClickLastSource !== 'window') {
    throw new Error(
...
liveMatch('a scrolled filler line still clicks the rare word inside the click wait', async () => {
  const fs = require('fs') as typeof import('fs');
  const sharp = require('sharp');
  const imageService = new ImageService('http://127.0.0.1:5001');
  expect(await imageService.healthCheck(800)).toBe(true);
  ...
  engine.config = {
    profileId: 'live-scrolled-filler',
    target: 'Terminal',
    useImageMatching: true,
    imageMatchThreshold: 0.6,
    timingTolerance: 20,
  ...
  const result = await engine.getSmartClickCoords(0, {
    ...
    img_context_b64: context.toString('base64'),
    metadata: {
      ocr_primary_text_normalized: 'filler 01',
      ocr_anchor_norm_x: 0.4,
      ocr_anchor_norm_y: 0.4,
    },
  }, point);
  ...
  captureSpy.mockRestore();
  screenSpy.mockRestore();
  const word = { x: 249, y: 665 };
  const nearWord = Math.abs(result.x - word.x) <= 12 && Math.abs(result.y - word.y) <= 12;
  if (!nearWord || elapsed >= 460 || status.smartClickLastMethod !== 'ocr' || !(status.smartClickLastConfidence > 0.6) || status.smartClickLastSource !== 'window') {
    throw new Error(
      `click (${result.x}, ${result.y}) word (${word.x}, ${word.y}) ${status.smartClickLastMethod} ${status.smartClickLastConfidence} source ${status.smartClickLastSource} elapsed ${elapsed}`
    );
...
liveMatch('a scrolled paragraph word still clicks when it is not inside a button', async () => {
  const fs = require('fs') as typeof import('fs');
  const sharp = require('sharp');
  const imageService = new ImageService('http://127.0.0.1:5001');
  expect(await imageService.healthCheck(800)).toBe(true);
  ...
  engine.config = {
    profileId: 'live-scrolled-paragraph',
    target: 'Terminal',
    useImageMatching: true,
    imageMatchThreshold: 0.6,
    timingTolerance: 20,
  ...
  const result = await engine.getSmartClickCoords(0, {
    ...
    img_context_b64: context.toString('base64'),
    metadata: {
      ocr_primary_text_normalized: 'deterministic',
      ocr_anchor_norm_x: 0,
      ocr_anchor_norm_y: 0,
    },
  }, point);
...
liveMatch('a paragraph word that crosses the neighborhood edge still clicks', async () => {
  const fs = require('fs') as typeof import('fs');
  const sharp = require('sharp');
  const imageService = new ImageService('http://127.0.0.1:5001');
  expect(await imageService.healthCheck(800)).toBe(true);
  const zoomedPng = await fs.promises.readFile(path.join(__dirname, 'fixtures/harness-100.png'));
  const meta = await sharp(zoomedPng).metadata();
  const frameW = meta.width ?? 1100;
  const frameH = meta.height ?? 800;
  const edgeScroll = 360;
  const shifted = await sharp({
    create: { width: frameW, height: frameH, channels: 3, background: { r: 245, g: 247, b: 251 } },
  })
    .composite([{ input: zoomedPng, left: 0, top: edgeScroll }])
    .png()
    .toBuffer();
  const patch = await sharp({
    create: { width: 128, height: 128, channels: 3, background: { r: 10, g: 20, b: 30 } },
...
liveMatch('a scrolled paragraph word at 100% still clicks when the line is wider than the slice cap', async () => {
  const fs = require('fs') as typeof import('fs');
  const sharp = require('sharp');
  const imageService = new ImageService('http://127.0.0.1:5001');
  expect(await imageService.healthCheck(800)).toBe(true);
  ...
  engine.config = {
    profileId: 'live-scrolled-paragraph-100',
    target: 'Terminal',
    useImageMatching: true,
    imageMatchThreshold: 0.6,
    timingTolerance: 20,
